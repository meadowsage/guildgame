...
public class AttributesTest {
    ...
    void test() {
        ...
        for (int i = 0; i < 40; i++) {
            int start = i * 5 + 1;
            int end = (i + 1) * 5;
            double ratio = (double) IntStream.of(results).filter(val -> (val >= start && val <= end)).count() / (n * 3d / 100);
        ...
        System.out.println(limit + "〜 : " +
                nf.format((double) IntStream.of(results).filter(val -> val > limit).count() / (n * 3d / 100)) + "%");
    ...
    void test2() {
        ...
        for (int i = 0; i < n; i++) {
            Attributes attributes = Attributes.generateRandom();
            System.out.println(attributes.getBattle() + " " + attributes.getKnowledge() + " " + attributes.getSupport());
        }
    }
