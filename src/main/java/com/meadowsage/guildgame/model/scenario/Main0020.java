...
public class Main0020 extends Scenario {
    ...
    public void afterProcess(GameWorld world, GameLogger gameLogger) {
        // 申請１件追加
        world.getApplicants().addAll(Applicant.generate(1, new Dice()));
    }
    ...
    public List<ScenarioScript> getScripts() {
        ...
        return Arrays.asList(
                ...
                , eris.speak("＜画像＞登録した冒険者は「冒険者」メニューからパーティに加えることができます。")
                , eris.speak("テランさんに休養をとってもらうため、今回は新しいパーティを結成しましょう。")
                , eris.speak("「新規パーティ結成」ボタンを押したら、下にある冒険者の一覧からメンバーを選択してください。")
                , eris.speak("「このメンバーで結成する」ボタンを押すと、パーティが保存されます。リーダー以外のメンバーは自由に変えられるので、クエストに応じて組み替えても大丈夫です。")
                , eris.speak("最後に、クエストを受注してもらうのを忘れないでくださいね。")
        );
    }
}
